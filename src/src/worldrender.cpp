...
// Begin Intel Corporation code
	...
	GLfixed c1, c2, f1, f2;
	GLfixed d14 = DivFX(i2x(d1->vdelta),FOUR_FX,true);
	GLfixed d24 = DivFX(i2x(d2->vdelta),FOUR_FX,true);
...
#ifdef _WIN32_WCE
// fixed point version
void render_seg_new(GLfixed vx, GLfixed vy, GLfixed vh, int mip, int x, int y, int xs, int ys)
{
    sqr *w = wmip[mip];
    int sz = ssize>>mip;
    int vxx = (x2i(vx)+(1<<mip)/2)>>mip;
    int vyy = (x2i(vy)+(1<<mip)/2)>>mip;
    int lx = vxx-lodleft;   // these mark the rect inside the current rest that we want to render using a lower mip level
    int ly = vyy-lodtop;
    int rx = vxx+lodright;
    int ry = vyy+lodbot;

	GLfixed p1x = f2x(player1->o.x);
	GLfixed p1y = f2x(player1->o.y);
	GLfixed fsize = ONE_FX<<mip;
	
	
	for(int ox = x; ox<xs; ox++) 
		for(int oy = y; oy<ys; oy++)       // first collect occlusion information for this block
    {
        SWS(w,ox,oy,sz)->occluded = isoccluded(p1x, p1y, i2x(ox)<<mip, i2x(oy)<<mip, fsize);
    };
    ...
    #define df(x) i2x(s->floor)-DivFX(i2x(x->vdelta), FOUR_FX,true)
    #define dc(x) i2x(s->ceil)+DivFX(i2x(x->vdelta), FOUR_FX,true)
    ...
    LOOPH   // ceils
        ...
        stats[mip]++;
        LOOPD
        if((s->type==SPACE || s->type==FHF) && s->ceil>=x2i(vh) && render_ceil)
    ...
    LOOPH continue;     // floors
        LOOPD
        if((s->type==SPACE || s->type==CHF) && s->floor<=x2i(vh) && render_floor)
        {
        ...
        if(s->type==FHF)
        {
			...
			if(x2i(df(s))<hdr.waterlevel && !SOLID(s)) addwaterquad(xx<<mip, yy<<mip, 1<<mip);
        };
    }};
...
// Begin Intel Corporation code
#ifdef _WIN32_WCE 
    render_seg_new(f2x(vx), f2x(vy), f2x(vh), MAX_MIP, 0, 0, ssize>>MAX_MIP, ssize>>MAX_MIP);
#else // End Intel Corporation code
